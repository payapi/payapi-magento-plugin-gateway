(function (window) {
 ...
 publicId };
    ...
    // sdk init() function
    payapiSdk.prototype.configure = function (publicId, key, isStaging) {
      if (!validatePublicIdFormat(publicId)) {
        ...
        this.domain = (isStaging) ? "https://staging-input.payapi.io" : "https://input.payapi.io";
      }
    };

    payapiSdk.prototype.addSecurePaymentButtonToDiv = function (jsonObjectGenerator, divClass, buttonText) {
      ...
      if (!validatePublicIdFormat(this.publicId) || !validateKeyFormat(this.key)) {
        ...
        button.setAttribute('class', 'payapi-payment-button');
        button.innerHTML = buttonText ? buttonText : 'Buy now';

        button.addEventListener(
            'click',
            function () {
            if (typeof jsonObjectGenerator == 'function') {
            var jsonObject = jsonObjectGenerator();
            if (!validateJson(jsonObject)) {
              console.warn('The JSON file format is wrong');
            } else {
              initSecureform(jsonObject, publicId, key, domain);
            }
            } else {
            console.warn('The JSONGenerator should be a function');
            }
            },
            true
        );
    ...
    payapiSdk.prototype.postData = function (jsonObject) {
        if (!validatePublicIdFormat(this.publicId) || !validateKeyFormat(this.key)) {
          console.warn('Configure your public Id and key before assing the payment button');
          return false;
        } else {
          initSecureform(jsonObject, this.publicId, this.key, this.domain);
        }

      
    }

    function initSecureform(jsonObject, publicId, key, domain)
    {
      ...
      form.setAttribute('action', domain + '/v1/secureform/'+ publicId);
